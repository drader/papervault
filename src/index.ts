...
async function main() {
  console.log('[papervault] Starting...')

  // Validate wiki on startup, report issues but don't block
  const issues = validateWiki()
  if (issues.length > 0) {
    console.warn(`[papervault] ${issues.length} wiki page(s) have schema issues:`)
  ...
  console.log('[papervault] Daemon running. Press Ctrl+C to stop.')
}

main().catch(err => {
  console.error('[papervault] Fatal error:', err)
  process.exit(1)
})
